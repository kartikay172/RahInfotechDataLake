...
args = getResolvedOptions(sys.argv, ["JOB_NAME","SOURCE_BUCKET","RAW_PREFIX","PROCESSED_PREFIX","CURATED_PREFIX","SECRET_NAME","REGION_NAME"])
...
PII_SALT       = json.loads(secret_value["SecretString"]).get("pii_salt", "default_salt")
...
def hash_value(v): return hashlib.sha256(f"{PII_SALT}:{v}".encode()).hexdigest()[:16]
...
for key in xlsx_keys:
    ...
    try:
        ...
        for sheet_name in wb.sheetnames:
            ...
            data = [
                [str(c).replace(chr(10)," | ").replace(chr(13),"") if c is not None else "" for c in r]
            ...
            s3.put_object(Bucket=SOURCE_BUCKET, Key=proc_key, Body=buf.getvalue().encode(), ContentType="text/csv")
            schema = {"source":key,"sheet":sheet_name,"processed_at":now.isoformat(),"partition":PARTITION,"zone":"processed",
                "columns":[{"name":h,"type":col_types.get(h,"string"),"pii":is_pii_name(h) or is_pii_date(h),
                    "pii_treatment":"masked" if is_pii_name(h) else "hashed" if is_pii_date(h) else "none"} for h in headers]}
            s3.put_object(Bucket=SOURCE_BUCKET, Key=schema_key, Body=json.dumps(schema,indent=2).encode(), ContentType="application/json")
            logger.info(f"PROCESSED: {proc_key} ({len(transformed)} rows)")
            table_key = f"{folder}/{stem}/{slug}"
            if table_key not in all_processed_data:
                all_processed_data[table_key] = {"headers":headers,"rows":[],"col_types":col_types}
            all_processed_data[table_key]["rows"].extend(transformed)
    except Exception as e:
        logger.error(f"Failed {key}: {e}")
        import traceback; traceback.print_exc()

logger.info("Writing CURATED zone...")
for table_key, table_data in all_processed_data.items():
    headers  = table_data["headers"]
    all_rows = table_data["rows"]
    col_types = table_data["col_types"]
    ...
    for row in all_rows:
        ...
        if rt not in seen:
            seen.add(rt)
            deduped.append(row)
    curated_key = f"{CURATED_PREFIX}{table_key}/{PARTITION}/data.csv"
    meta_key    = f"{CURATED_PREFIX}{table_key}/{PARTITION}/metadata.json"
    buf = io.StringIO()
    w = csv.writer(buf)
    w.writerow(headers)
    w.writerows(deduped)
    s3.put_object(Bucket=SOURCE_BUCKET, Key=curated_key, Body=buf.getvalue().encode(), ContentType="text/csv")
    metadata = {"zone":"curated","table":table_key,"partition":PARTITION,"processed_at":now.isoformat(),
        "total_rows":len(all_rows),"curated_rows":len(deduped),"duplicates_removed":len(all_rows)-len(deduped),
        "columns":[{"name":h,"type":col_types.get(h,"string")} for h in headers]}
    s3.put_object(Bucket=SOURCE_BUCKET, Key=meta_key, Body=json.dumps(metadata,indent=2).encode(), ContentType="application/json")
    logger.info(f"CURATED: {curated_key} ({len(deduped)} rows)")

logger.info(f"Done. RAW={RAW_PREFIX} PROCESSED={PROCESSED_PREFIX} CURATED={CURATED_PREFIX}")